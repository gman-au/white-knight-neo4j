...
namespace White.Knight.Neo4J.Relationships
{
    ...
    public static class RelationshipNavigation
    {
        ...
        public static IEnumerable<IRelationshipNavigation> GetNavigationChain(this IRelationshipNavigation relationshipNavigation)
        {
            yield return relationshipNavigation;
            var next = relationshipNavigation.Next();
            if (next == null || next == Empty) yield break;
            yield return next;
        }
    }
}